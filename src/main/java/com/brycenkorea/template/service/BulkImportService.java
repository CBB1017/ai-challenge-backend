...
public class BulkImportService {
    ...
    public Mono<BulkImportResult> importMembers(List<MemberRequest> memberRequests) {
        return Flux.fromIterable(memberRequests)
            .flatMap(dto -> memberService.saveOrUpdateByEmail(dto).thenReturn(true) // 성공 시 true
    ...
    public Mono<BulkImportResult> importAttendance(Map<String, List<AttendanceDto>> teamData) {
        ...
        return resolveMemberMap(teamData).flatMap(memberMap -> {
            ...
            List<AttendanceDto> allDtos = teamData.values().stream().flatMap(List::stream).toList();

            return Flux.fromIterable(allDtos).flatMap(dto -> {
                String key = dto.getMemberName() + "::" + dto.getPosition();
                Member member = memberMap.get(key);

                if (member == null) {
                    String error = "멤버 조회 없음: " + key;
                    return sendNotificationIfRequired(null, dto.getStatus(), error).thenReturn(BulkResult.fail(
                        key,
                        error
                    ));
                }
                ...
                return sendNotificationIfRequired(member, dto.getStatus(), null).thenReturn(BulkResult.success(entity));
            }).collectList().flatMap(results -> {
                // 1. 성공한 엔티티 리스트 추출 (성공 여부는 r.success()로 체크)
                List<Attendance> toSave = results.stream().filter(BulkResult::success) // success() 메서드가 true인 것만
    ...
    private Mono<Void> sendNotificationIfRequired(Member member, AttendanceStatus status, String errorReason) {
        if (status != null && status.name().endsWith("_NOTIFICATION")) {
            ...
            NotificationSendDto notifyDto = new NotificationSendDto(
                member,
                status,
                status.getMessage(),
                result,
    ...
    private Mono<Map<String, Member>> resolveMemberMap(Map<String, List<AttendanceDto>> teamData) {
        ...
        return memberService.findAllByNameInAndPositionIn(nameSet, positionSet).collectMap(
