...
public class LoginService {
    ...
    public Mono<ResponseEntity<Map<String, String>>> processUserSession(
        LoginResponse.CrawlerUser userData,
    ...
    )
    {
        ...
        if (expiry == null) {
            throw new IllegalArgumentException("Cookie max-age not found");
        }
        ...
        return userRepository.findByEmail(email)
            ...
            .map(user -> {
                ...
                ResponseCookie cookie = ResponseCookie.from("accessToken", myToken)
                    .maxAge(Duration.between(Instant.now(), finalExpiry.toInstant()))
                    .httpOnly(true)
                    .path("/")
                    .build();
    ...
    public Mono<ResponseEntity<Map<String, String>>> checkUserSession(String email) {
        ...
        return userRepository.findByEmail(email).mapNotNull(user -> ResponseEntity.ok().body(Map.of(
            "email",
            user.getEmail(),
            "name",
            user.getName(),
