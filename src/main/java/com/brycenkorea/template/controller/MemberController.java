...
public class MemberController {
    ...
    public Mono<CommonResponse<MemberResponse>> create(@RequestBody @Valid MemberRequest memberDto) {
        return memberService.save(memberDto).map(member -> CommonResponse.ok(memberResponseMapper.toResponse(member)));
    }
    ...
    public Mono<CommonResponse<MemberResponse>> read(@PathVariable @Positive Long id) {
        return memberService.findById(id).map(member -> CommonResponse.ok(memberResponseMapper.toResponse(member)));
    }

    @Operation(summary = "회원 전체 목록 조회", description = "모든 회원을 조회합니다.")
    @GetMapping
    public Mono<CommonResponse<List<MemberResponse>>> readAll() {
        return memberService.findAll().collectList() // Flux<Member>를 Mono<List<Member>>로 변환
    ...
    @GetMapping("/search")
    public Mono<CommonResponse<List<MemberResponse>>> readAllWithSearch(
        Pageable pageable,
    ...
    public Mono<CommonResponse<Void>> delete(@PathVariable @Positive Long id) {
        return memberService.deleteById(id).thenReturn(CommonResponse.ok());
    }
}