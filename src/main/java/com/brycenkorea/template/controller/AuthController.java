...
public class AuthController {
    ...
    public Mono<ResponseEntity<?>> performLogin(@RequestBody LoginRequest loginRequest) {
        return pythonCrawlerWebClient.post().uri("/api/login").bodyValue(loginRequest).retrieve().onStatus(
                HttpStatusCode::is4xxClientError, response -> response.bodyToMono(String.class).flatMap(body -> {
