...
public record CommonResponse<T>(int code, String message, T data, Object error, String traceId) {
    ...
    public static <T> CommonResponse<T> error(ApiResultCode code, ErrorDetail errorDetail) {
        var ctx = Span.current().getSpanContext();
        return new CommonResponse<>(code.getCode(), code.getMessage(), null, errorDetail, ctx.getTraceId());
    }
